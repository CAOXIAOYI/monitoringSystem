...
class TopologySearchCtrl {
  ...
  search(keyword) {
    ...
    for (let i = 0; i < this.cache.length && result.length < 20; i++) {
      if ((this.cache[i].n.toLocaleLowerCase().indexOf(keyword) !== -1) || (this.cache[i].d && this.cache[i].d.toLocaleLowerCase().indexOf(keyword) !== -1)) {
...
export default TopologySearchCtrl;
