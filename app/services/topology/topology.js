...
class TopologyCtrl {
  ...
  _dealWithData(data) {
    let result = {};
    result.nodes = data.vertexs && data.vertexs.map((item) => {
      return {
        des: item.d,
        name: item.n,
        id: item.i
    ...
    result.links = data.edges && data.edges.map((item) => {
      return {
    ...
    result.nodes && result.nodes.forEach((node) => {
      let count = 0;
  ...
  locatePosition(item) {
    ...
    this.nodes && this.nodes.forEach((node) => {
      if (node.name === item.n) {
        locateDom = node;
      }
    });
  ...
  hideAll() {
    ...
    this.nodes && this.nodes.forEach((item) => {
      if (item.isFocus) {
    ...
    //隐藏所有node
    hideNodes && hideNodes.forEach((node) => {
      d3.select(node._element)
        .style("display", "none");
    });

    //隐藏所有link
    hideLinks && hideLinks.forEach((link) => {
  ...
  findNode(id) {
    let result = this.nodes && this.nodes.filter((item) => {
      return item.id === id;
    })
